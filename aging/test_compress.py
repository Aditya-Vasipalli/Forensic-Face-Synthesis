...
def _log_event(
    event: str, *, context: UploadContext, path: str, **extra: object
) -> None:
...
def build_context(request: Any | None = None) -> UploadContext:
    ...
    return UploadContext(
        session=session, ip=ip, country=country, agent=agent, extras=extras
    )


def imagine(
    local_path: str, age: int, *, context: UploadContext | None = None
) -> str | None:
    ...
    try:
        ...
        content_type = (
            "image/webp" if prepared_upload.format_name == "webp" else "image/jpeg"
        )
        s3.upload_file(
        ...
        print(
            f"Uploaded {source_name} ({prepared_upload.format_name}) to s3://{BUCKET_NAME}/{key}"
        )
        ...
        if (
            prepared_upload
            and prepared_upload.cleanup
            and os.path.exists(prepared_upload.path)
        ):
            try:
                os.remove(prepared_upload.path)
            except OSError as cleanup_error:
                print(
                    f"Warning: could not delete temp file {prepared_upload.path}: {cleanup_error}"
                )
