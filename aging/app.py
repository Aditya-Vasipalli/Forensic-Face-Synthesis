...
# Model path — use local file if present, otherwise download from HuggingFace
_HERE = os.path.dirname(os.path.abspath(__file__))
_LOCAL_MODEL = os.path.join(_HERE, "best_unet_model.pth")

MODEL_DIR = os.environ.get("MODEL_DIR", os.path.join(_HERE, "model_cache"))
os.makedirs(MODEL_DIR, exist_ok=True)
MODEL_PATH = (
    _LOCAL_MODEL
    if os.path.exists(_LOCAL_MODEL)
    else os.path.join(MODEL_DIR, "best_unet_model.pth")
)
...
def age_image(
    image_path: str, source_age: int, target_age: int, request: gr.Request | None = None
) -> Image.Image:
    try:
        ...
        if "No faces detected" in str(e):
            raise gr.Error(
                "No faces detected in the image. Please upload an image with a clear, visible face."
            )
        else:
...
def age_timelapse(
    image_path: str, source_age: int, request: gr.Request | None = None
) -> str:
    try:
        context = build_context(request)
        image = Image.open(image_path)
...
demo_age_image = gr.Interface(
    ...
    inputs=[
        ...
        gr.Slider(
            0, 90, value=20, step=1, label="Current age", info="Choose the current age"
        ),
        gr.Slider(
            0, 90, value=70, step=1, label="Target age", info="Choose the desired age"
        ),
    ],
...
demo_age_video = gr.Interface(
    ...
    inputs=[
        ...
        gr.Slider(
            label="Frames per second (fps)", minimum=2, maximum=60, step=1, value=30
        ),
    ],
...
demo_age_timelapse = gr.Interface(
    ...
    inputs=[
        gr.Image(type="filepath", label="Input Image"),
        gr.Slider(0, 90, value=20, step=1, label="Current age"),
    ],
